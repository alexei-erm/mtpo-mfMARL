...
class EpisodesBufferEntry:
    ...
    def append(self, view, feature, action, reward, alive, prob0=None,  prob1=None):
...
class EpisodesBuffer(Buffer):
    ...
    def push(self, **kwargs):
        ...
        for i in range(len(ids)):
            ...
            if self.use_mean:
                entry.append(view[i], feature[i], acts[i], rewards[i], alives[i], prob0=prob0[i], prob1=prob1[i])
            else:
...
class Runner(object):
    ...
    def run(self, variant_eps, iteration, win_cnt=None):
        ...
        for i, tag in enumerate(['main', 'opponent1']):
